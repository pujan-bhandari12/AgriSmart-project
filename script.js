...
async function turnPumpOn() {
    await fetch(`https://blynk.cloud/external/api/update?token=${BLYNK_AUTH_TOKEN}&${PUMP_PIN}=1`);
}
async function turnPumpOff() {
    await fetch(`https://blynk.cloud/external/api/update?token=${BLYNK_AUTH_TOKEN}&${PUMP_PIN}=0`);
}
